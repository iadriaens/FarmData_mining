function OUT = DLV_milkdata_v37(cd,FN_BA,FN_ALS,FN_AHD,FN_SMY,FN_VMY,cd_H,temp_dir)
...
fid = fopen([temp_dir 'FN_AHD.txt'],'r'); f=fread(fid,'*char')'; fclose(fid);
f=f(1:length(f)-1);
fid = fopen([temp_dir 'FN_AHD.txt'],'w');fwrite(fid,f); fclose(fid);
...
fid  = fopen([temp_dir 'FN_VMY.txt'],'r'); f=fread(fid,'*char')'; fclose(fid);
    f = strrep(f,'=ja;','deletesemicolon');         % delete semicolon in Voluntary Session Milk Yield
    f = strrep(f,';TotalFlow','deletesemicolon');   % delete semicolon in Voluntary Session Milk Yield
    f = strrep(f,';KFC','deletesemicolon');         % delete semicolon in Voluntary Session Milk Yield
    f = strrep(f,';MsEv','deletesemicolon');f=f(1:length(f)-1); % delete semicolon in Voluntary Session Milk Yield
fid  = fopen([temp_dir 'FN_VMY.txt'],'w'); fwrite(fid,f);fclose(fid);


clear als_H ba_H ahd_H smy_H vmy_H ans
...
% clear variables
clear F1 F2 F3 F4 FN j DT1 DT2 FN opts
...
idx_AHD = zeros(1,length(col_AHD));       % to fill in - column indices
idx_SMY = zeros(1,length(col_SMY));       % to fill in - column indices
idx_VMY = zeros(1,length(col_SMY));      % to fill in - column indices
...
c.Properties.VariableNames = {'OID2','BeginTime','BA','DIM','Lac','EndTime','PEndTime'};  % AHD
d.Properties.VariableNames = {'OID2','TMY','Dest','SesNo'};      % SMY
e.Properties.VariableNames = {'OID2','MYLF','MYRF','MYLR','MYRR','ECLF','ECRF','ECLR','ECRR','BloodLF','BloodRF','BloodLR','BloodRR','PFLF','PFRF','PFLR','PFRR','MFLF','MFRF','MFLR','MFRR','MDI','NotMilkedTeats','Incomplete','Kickoff','MilkType'};

% clear variables
clear idx_ALS idx_BA idx_AHD idx_VMY idx_SMY exttype opts ans
...
ind = find(isnat(OUT.BeginTime)==1);

cowlac = sortrows(unique(OUT(:,[3 5 41]),'rows'),1);  % BA Lac Calving
for i = 1:length(cowlac.BA(:,1))
    ind = find(OUT.BA == cowlac.BA(i) & OUT.Lac == cowlac.Lac(i));
    cowlac.StartDate(i,1) = min(OUT.BeginTime(ind));
    cowlac.StartDIM(i,1) = min(OUT.DIM(ind));
    cowlac.StartDIMDate(i,1) = OUT.DIM(ind(OUT.BeginTime(ind) == min(OUT.BeginTime(ind))));
    cowlac.EndDate(i,1) = max(OUT.BeginTime(ind));
    cowlac.EndDIM(i,1) = max(OUT.DIM(ind));
    cowlac.EndDIMDate(i,1) = OUT.DIM(ind(OUT.BeginTime(ind) == max(OUT.BeginTime(ind))));
    ...
    cowlac.Nmeas(i,1) = length(ind);
    cowlac.Ndays(i,1) = datenum(cowlac.EndDate(i,1))-datenum(cowlac.StartDate(i,1));
    cowlac.diff3(i,1) = cowlac.Nmeas(i,1)/cowlac.Ndays(i,1); 

end

% delete all data for which only one measurement is available
cowkeep = cowlac(cowlac.Nmeas > 2,[1 2]);
OUT = innerjoin(OUT, cowkeep,'Keys',{'BA','Lac'});


clear dates dates1 dates2 dates3 ind test cowdel cowkeep cows DIM i ind sub sub2 OUT2 ans
...
% % SUM.Ndays(1,1) = datenum(max(OUT.BeginTime))-datenum(min(OUT.BeginTime)); 
