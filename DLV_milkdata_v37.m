function OUT = DLV_milkdata_v37(cd,FN_BA,FN_ALS,FN_AHD,FN_SMY,FN_VMY,cd_H)
...
fid = fopen([newdir 'FN_AHD.txt'],'r'); f=fread(fid,'*char')'; fclose(fid);
f=f(1:length(f)-1);
fid = fopen([newdir 'FN_AHD.txt'],'w');fwrite(fid,f); fclose(fid);
